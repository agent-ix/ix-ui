...
|----|----------|--------------|
| FR-006-AC-1 | All prompts render their `message` on a header line prefixed by a cyan `?` glyph at the prompt's left margin (column 1 when `render()` is the parent; column `ROW_INDENT` when rendered inside a `<Frame>` / `<Listing>` body, since the parent body box places children at that indent) | Test |
| FR-006-AC-2 | All prompts SHALL handle Ctrl-C / Esc by calling `onSubmit({ ok: false, cancelled: true })` and unmounting their input | Test |
| FR-006-AC-3 | All prompts SHALL handle Enter as confirmation, validate (where applicable), and call `onSubmit({ ok: true, value })` when valid | Test |
| FR-006-AC-4 | After submission (success or cancel), the prompt SHALL render a frozen one-line summary: `? <message> <result-or-«cancelled»>` | Test |
| FR-006-AC-5 | `<TextPrompt>` SHALL render an editable text field via `ink-text-input` | Test |
| FR-006-AC-6 | The submitted summary renders the entered value (or `«empty»` dim if empty and accepted) | Test |
| FR-006-AC-7 | `<PasswordPrompt>` SHALL render the input with each character replaced by `•` | Test |
| FR-006-AC-8 | The submitted summary renders `••••••••` (8 bullets fixed) regardless of input length | Test |
| FR-006-AC-9 | `<ConfirmPrompt>` SHALL render `(Y/n)` or `(y/N)` per `defaultValue` | Test |
| FR-006-AC-10 | The submitted summary renders `Yes` or `No` (capitalized) | Test |
| FR-006-AC-11 | `<SelectPrompt>` SHALL render the option list via `ink-select-input` with the cyan caret marking the active option | Test |
| FR-006-AC-12 | When an option has a `hint`, it renders dim to the right of the label with two-space separation | Test |
| FR-006-AC-13 | `<MultiSelectPrompt>` SHALL render the option list with `[ ]` / `[•]` checkboxes | Test |
| FR-006-AC-14 | When `required === true` and no option is selected at submit, validation error `"select at least one option"` renders and the prompt re-arms | Test |
| FR-006-AC-15 | Prompts MAY be rendered as children of `<Listing>` or `<Frame>` | Test |
| FR-006-AC-16 | When multiple prompts are present in a tree, only the most recently mounted prompt receives input | Test |
| FR-006-AC-17 | When the terminal is resized while a prompt is active, the prompt SHALL re-flow without losing the in-progress input value | Test |
...
| FR-006-AC-21 | If the consumer-supplied `onSubmit` callback throws (synchronously or returns a rejected Promise), the prompt SHALL still render its frozen summary | Test |
...
| FR-006-AC-24 | On that same `onSubmit` throw, the thrown error SHALL propagate up to the surrounding error boundary or `render()` ([FR-008-AC-13](./FR-008-render-entry-point.md)) rather than being swallowed | Test |
...
- **FR-006-AC-21**: If the consumer-supplied `onSubmit` callback throws (synchronously or returns a rejected Promise), the prompt SHALL still render its frozen summary.
- **FR-006-AC-24**: On that same throw, the thrown error SHALL propagate up to the surrounding error boundary or `render()` ([FR-008-AC-13](./FR-008-render-entry-point.md)). The prompt SHALL NOT swallow the error or re-arm itself in response.
