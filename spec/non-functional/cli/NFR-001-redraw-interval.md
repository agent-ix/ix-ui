...
|----|----------|--------------|
| NFR-001-AC-1 | The shared `useInterval(tick, 80)` hook (FR-007) drives every animated frame in the package. Components requiring animation SHALL consume this hook (or a wrapper that uses it) — no raw `setInterval` calls exist in component code. | Demonstration |
